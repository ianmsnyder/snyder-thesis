...
The purpose of this is to check the analysis dependence on pileup.  Ideally there is no dependence.  The preselection is the same as in Section \ref{sec:srDefs}.% which the exeptions $\met > 200 \gev$ and no \mtbmin\ cut.  This is to increase statistics in data.
...
The purpose of this check is to check the pileup reweighting.  Ideally the Monte Carlo will match the data after reweighting.  The preselection is the same as in Section \ref{sec:srDefs}.% which the exeptions $\met > 200 \gev$ and no \mtbmetmindphi\ cut or number of b-jets.  The plot demonstrates that the Monte Carlo is close to data after reweighting.
